...
import { useRoles, useUserRoles, useAssignRoleToUser, useRevokeRoleFromUser } from '@/hooks/useApi'
...
export function UserRolesDialog({ open, onOpenChange, user }: UserRolesDialogProps) {
  ...
  const { data: userRolesData } = useUserRoles(
    user ? { user: user.pulp_href, limit: 100 } : undefined
  )
  ...
  // Get current role hrefs from user's role assignments
  const currentRoleHrefs = new Set(
    userRolesData?.results?.map((ur: PulpUserRole) => ur.role) || []
  )
  ...
  userRolesData?.results?.forEach((ur: PulpUserRole) => {
    roleToAssignmentMap.set(ur.role, ur.pulp_href)
  })

  const handleAssignRole = (roleHref: string) => {
    if (user) {
      assignMutation.mutate({ user: user.pulp_href, role: roleHref })
    }
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Assign or revoke roles for "{user.username}"
          </DialogDescription>
        </DialogHeader>
